...
Namespace Validation
    ...
    Public NotInheritable Class ValidationService
        ...
        Public Function Validate(item As OutboxRecord) As ValidationResult Implements IValidationService.Validate
            ...
            RequireDateTime14(result, "CONTRATTO_CHECKOUT_DATA", item.ContrattoCheckoutData)
            ...
            RequireDateTime14(result, "CONTRATTO_CHECKIN_DATA", item.ContrattoCheckinData)
            ...
            ValidateLength(result, "CONTRATTO_ID", item.ContrattoId, 10)
            ...
            ValidateLength(result, "CONTRATTO_CHECKOUT_INDIRIZZO", item.ContrattoCheckoutIndirizzo, 80)
            ValidateLength(result, "CONTRATTO_CHECKIN_LUOGO_COD", item.ContrattoCheckinLuogoCod, 9)
            ValidateLength(result, "CONTRATTO_CHECKIN_INDIRIZZO", item.ContrattoCheckinIndirizzo, 80)
            ValidateLength(result, "OPERATORE_ID", item.OperatoreId, 9)
            ValidateLength(result, "AGENZIA_ID", item.AgenziaId, 9)
            ValidateLength(result, "AGENZIA_NOME", item.AgenziaNome, 38)
            ValidateLength(result, "AGENZIA_LUOGO_COD", item.AgenziaLuogoCod, 9)
            ValidateLength(result, "AGENZIA_INDIRIZZO", item.AgenziaIndirizzo, 80)
            ValidateLength(result, "AGENZIA_RECAPITO_TEL", item.AgenziaRecapitoTel, 15)
            ValidateLength(result, "VEICOLO_TIPO", item.VeicoloTipo, 2)
            ValidateLength(result, "VEICOLO_MARCA", item.VeicoloMarca, 20)
            ValidateLength(result, "VEICOLO_MODELLO", item.VeicoloModello, 20)
            ValidateLength(result, "VEICOLO_TARGA", item.VeicoloTarga, 10)
            ValidateLength(result, "CONDUCENTE_CONTRAENTE_COGNOME", item.ConducenteContraenteCognome, 38)
            ValidateLength(result, "CONDUCENTE_CONTRAENTE_NOME", item.ConducenteContraenteNome, 38)
            ValidateLength(result, "CONDUCENTE_CONTRAENTE_NASCITA_LUOGO_COD", item.ConducenteContraenteNascitaLuogoCod, 9)
            ValidateLength(result, "CONDUCENTE_CONTRAENTE_CITTADINANZA_COD", item.ConducenteContraenteCittadinanzaCod, 3)
            ValidateLength(result, "CONDUCENTE_CONTRAENTE_DOCIDE_TIPO_COD", item.ConducenteContraenteDocideTipoCod, 2)
        ...
        Private Shared Sub RequireDateTime14(result As ValidationResult, fieldName As String, value As Nullable(Of DateTime))
            If Not value.HasValue Then
                result.MissingFields.Add(fieldName)
            End If
        End Sub
