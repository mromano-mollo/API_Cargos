/*
    Template for dbo.Cargos_Vista_Contratti_PARMIANI
*/

CREATE OR ALTER VIEW [dbo].[Cargos_Vista_Contratti_PARMIANI]
...
SELECT
  ...
  FROM BC.dbo.[Parmiani$AR Contract Header] HCTR WITH(NOLOCK)

  LEFT JOIN BC.dbo.[Parmiani$AR Contract Line] LCTR WITH(NOLOCK)
    ON HCTR.[Contract Type] = LCTR.[Contract Type]
   AND HCTR.[Contract No_] = LCTR.[Contract No_]

  LEFT JOIN [BC-SQLSRV01].[PARMIANI].[dbo].[Parmiani$Contact$437dbf0e-84ff-417a-965d-ed2bb9650972] C WITH(NOLOCK)
    ON C.[no_] = HCTR.[Cargos Contatto 1]

  LEFT JOIN [BC-SQLSRV01].[PARMIANI].[dbo].[Parmiani$Contact$437dbf0e-84ff-417a-965d-ed2bb9650972$ext] CExt WITH(NOLOCK)
    ON CExt.[no_] = C.[no_]
  ...
  LEFT JOIN BC.dbo.[Parmiani$AR Object Card] Obj WITH(NOLOCK)
    ON Obj.[Object No_] = LCTR.No_

  LEFT JOIN BC.dbo.[Parmiani$AR Object Type] ObjType WITH(NOLOCK)
